...
void test03()
{
	...
	LevelOrderTraverse(mybitree);
	printf("functon of LevelOrderTraverse successed\r\n");
...
void test04()
{
	HuffmanTree* myhuffmantree=TreesArrInit(4);
	createHuffmanTree(myhuffmantree);
	HuffmanTree* testtree = myhuffmantree;
	for(int i=0;i<7;i++)
	{
		testtree++;
		printf("%d\r\n", testtree->weight);
	...
	//printf("%d\r\n", testtree->weight);
	if (myhuffmantree->weight == 18)
	{
		printf("createhuffmantree successful\r\n");
	}
}
